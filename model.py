...
NUM_CLASSES = 13  # 14 classes de comportements (0 à 13)
...
class MovementClassifier(nn.Module):
    def __init__(self, mocap_features, pressure_features, hidden_dim=128, num_classes=14):
        ...
        #premier réseau pour motion capture
        self.mocap_cnn = nn.Sequential(
         ...
         #2nd réseau pour motion capture
...
def normalize_data_mocap(data):
    """
    Recentre les données par rapport au pelvis, supprime les 3 dernières dimensions,
    et normalise avec gestion des valeurs extrêmes pour éviter l'instabilité numérique.
    
    Args:
        data (numpy.ndarray): Tableau d'entrée de forme (273, 100, 129)
        
    Returns:
        tuple: (données normalisées de forme (273, 100, 126), paramètres de normalisation (mean, std))
    """
    # Vérification de la forme du tableau d'entrée
    if data.shape[2] < 129:
        raise ValueError(f"La troisième dimension doit être au moins 129, mais est {data.shape[2]}")
    
    # Vérifier et gérer les valeurs NaN avant la normalisation
    if np.isnan(data).any():
        print("Warning: NaN values found in data before normalization")
    ...
    # Extraire les coordonnées du pelvis (situées aux indices 126, 127, 128)
    pelvis_x = data[:, :, 126:127]  # Forme: (273, 100, 1)
    pelvis_y = data[:, :, 127:128]  # Forme: (273, 100, 1)
    pelvis_z = data[:, :, 128:129]  # Forme: (273, 100, 1)
    
    # Recentrer chaque coordonnée x, y, z par rapport au pelvis
    # On suppose que les coordonnées sont organisées comme 
    # [joint1_x, joint1_y, joint1_z, joint2_x, ...] avec indices respectifs [0, 1, 2, 3, ...]
    for i in range(0, 126, 3):  # Parcourir les indices x par pas de 3
        data_centered[:, :, i] -= pelvis_x[:, :, 0]  # x coordinates
        data_centered[:, :, i+1] -= pelvis_y[:, :, 0]  # y coordinates
        data_centered[:, :, i+2] -= pelvis_z[:, :, 0]  # z coordinates
    
    # Supprimer les 3 dernières dimensions (pelvis_x, pelvis_y, pelvis_z)
    data_reduced = data_centered[:, :, :126]
    
    # Clipping des valeurs extrêmes pour éviter l'instabilité numérique
    q_low = np.percentile(data_reduced, 1, axis=(0, 1), keepdims=True)
    q_high = np.percentile(data_reduced, 99, axis=(0, 1), keepdims=True)
    data_clipped = np.clip(data_reduced, q_low, q_high)
    
    # Normalisation (z-score) pour chaque feature
    mean = np.mean(data_clipped, axis=(0, 1), keepdims=True)
    std = np.std(data_clipped, axis=(0, 1), keepdims=True)
    # Assurer que std n'est pas trop petit
    std = np.maximum(std, 1e-6)
    
    normalized_data = (data_clipped - mean) / std
    
    # Vérifier et gérer les valeurs NaN après la normalisation
    if np.isnan(normalized_data).any():
        print("Warning: NaN values found after normalization")
        # Remplacer les valeurs NaN restantes par 0
        normalized_data = np.nan_to_num(normalized_data)
...
def main():
    ...
    try:
        ...
        mocap_data, mocap_stats = normalize_data_mocap(mocap_data)
